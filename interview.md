...
- [ ] 进程、线程、协程
  ...
  - （协程有自己的寄存器上下文和栈，切换的时候会有保存和恢复，一般栈就2KB，可以动态伸缩，线程栈大概要2MB）
...
- [ ] 缺页中断
- [ ] 进程间通信方式有哪些
- [ ] 共享内存怎么实现的，两个进程同时操作有什么问题，如何解决进程间的并发同步
- [ ] 原子操作底层原理
- [ ] 为什么操作系统所有进程都要设计父进程？ -->
<!-- - [ ] 线程池的核心参数 -->
...
- [ ] GMP模型
  - G即goroutine，存执行时的堆栈信息，状态以及任务函数等；P表示processer，P的数量决定了系统内最大可并行G的数量,M可以理解成内核线程。线程想运行任务要从P的本地队列获取G，如果本地队列为空，要从其他P的本地队列偷取或者从全局队列获取。如果一个M阻塞，P会去切换后者创建另一个M
...
- goroutine协作
  ...
  - [ ] 协程池
    - 一个结构体保存最大协程数、当前协程数、协程池状态，channel（传函数和参数），锁。提供协程池的创建关闭、任务提交、协程增减。
- I/O多路复用
...
- [ ] **redis数据类型**
  - string（缓存结构体消息，计数，字符串结构一般是buf+flags，flages前3位存类型，后5位存长度，存不下则预留，用len和alloc存长度和总长度）
...
- [ ] redis hash表
  ...
  - 扩展收缩
    ...
    - 分多次渐进rehash，防止服务停止
<!-- - [ ] ziplist/quicklist
  - 是Redis list、hash、zset的底层实现结构之一，当list、hash、zset中节点数量较少，并且存储的大多节点为小整数型，较短的字符串时，Redis就会使用ziplist作为list、hash、zset的底层实现。
  - 不存储指向上一个节点和下一个节点的指针，而是存上一个节点和当前节点的长度。存上一个节点长度是因为压缩列表数据不规则紧邻，无法通过后退指针找到上一个元素，通过当前地址减去上一个节点的长度可以找到上一个节点的位置向前回溯。 -->
- [ ] redis提高数据库容量的方式
...
- [ ] **redis持久化**
  - 分为RDB和AOF以及混合三种。RDS是默认存储方式，保存某一时刻数据的快照，符合自定规则/执行save、bgsave（阻塞时间短）、flushall命令/第一次主从复制会触发快照，优点是占用空间小，性能好，缺点是会丢失最后一次快照以后的数据；AOF记录写过的所有命令，按顺序回放进行恢复，如果AOF体积过大，会自动重写成恢复当前数据集的最小的命令集合，缺点主要是性能差，不建议单独使用。
- [ ] 持久化数据和缓存扩容
...
- [ ] redis事务
  - 不是传统意义上的事务，只是一个批量执行脚本，一条命令失败不会导致之前的命令回滚，之后的命令也依然会执行。
- [ ] **常见使用方式**
  - 单副本：简单方便性价比高；不保证可靠性
...
- 事务
  ...
  - [ ] mysql的四种隔离级别
    ...
    - 可重读：mysql默认的级别，同一事务的多个实例并发读数据，会看到同样的数据行，但会有幻读。
      - 在第一次select的时候获取read view就是可重复读，每次select都获取就是读已提交
...
- 日志
  ...
  - [ ] 执行顺序
    ...
    - 如果不用两阶段提交，假如先写redo log并且写完后崩溃，如果用binlog来恢复临时库，就会少一次更新；假如先写bin log，如果写完崩溃，该事务其实无效，但是binlog恢复会多一个事务出来。如果使用两阶段，binlog后崩溃，先检查redo log是否存在且完整，如果有问题就回滚日志。
  - [ ] WAL 预写日志
